...
static int GetProcessesDirect(void) {
  ...
  for (i = j = 0; i < globbuf.gl_pathc; i++) {
    ...
    snprintf(name, sizeof(name), "%s%s",
	     globbuf.gl_pathv[globbuf.gl_pathc - i - 1], "/comm");
    tn = fopen(name, "r");
    if (tn == NULL) continue; /* process vanished since glob() */
...
char *out_child;
/**
	Print only root (parent) processes in JSON format
*/
char *printRootProc(){
	cJSON *json;
	...
	for(i=0;i<NProc;i++){
		if(P[i].child != -1){
//			char *aux=strtok(P[i].cmd,"\n");
			cJSON_AddNumberToObject(json,strtok(P[i].cmd,"\n"),P[i].pid);
			//cJSON_AddNumberToObject(json,P[i].cmd,P[i].pid);
		}
	}

	out_root=cJSON_Print(json);

	/*for(i=0;i<strlen(out_root);i++){
		if(out_root[i] == '\n'){
			out_root[i] = ' ';
		}
	}*/
...
/**
	Print child processes from a parent PID
*/
char *printProcChild(int pid){
	cJSON *json;
	...
	for (child = P[idx].child; EXIST(child); child = P[child].sister) {
		...
		cJSON_AddNumberToObject(json,strtok(P[child].cmd,"\n"),P[child].pid);

  	}
//	char *out;
	out_child=cJSON_Print(json);
	cJSON_Delete(json);
//	printf("%s\n", out_child);
//	free(out);
return out_child;
...
void serverFunction(){
    ...
    puts("Connection accepted");
		char *send;// = printRootProc();
		int i;
//		write(client_sock,send, strlen(send));
		//fflush(stdout);
    //Receive a message from client
    while( (read_size = recv(client_sock , client_message , 3000 , 0)) > 0 )
    {
        //Send the message back to client
				printf("%s", client_message);

				if(strcmp(client_message, "proc") == 0){
						send=printRootProc();
						

						for(i=0;i<(strlen(send)-2);i++){
							if(send[i] == '\n'){
								send[i] = ' ';
							}
						}

			        	write(client_sock , send , strlen(send)+1);
						
						fflush(stdout);
				}else if((pid=atoi(client_message)) > 0){
						send=printProcChild(pid);
						for(i=0;i<(strlen(send)-2);i++){
							if(send[i] == '\n'){
								send[i] = ' ';
							}
